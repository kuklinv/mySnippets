...
function filteredArray(arr, elem) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].indexOf(elem) == -1) {
      ////  вот оно. фильтруем находясь на этом первом уровне через for
      newArr.push(arr[i]);
    }
  }
  return newArr;
...
function countOnline(obj) {
  ...
  for (let key in obj) {
    if (obj[key]["online"] == true) {
      // !!!!!! this a lot of minits requaire for understand
      counter += 1;
    }
  }
  return counter;
...
function allNumsToString(obj) {
  ...
  function flattenDeep(arr1) {
    return arr1
      .reduce(
        (acc, val) =>
          Array.isArray(val) ? acc.concat(flattenDeep(val)) : acc.concat(val), ////// !!!!!!!! recursion
        []
      )
      .sort();
  }

  return flattenDeep(arr).join(";");
}
// //input
//   {
//     "nums":[1,2,3],
